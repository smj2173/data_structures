...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
      double xcoordinate = Math.pow(ux-vx, 2);
      double ycoordinate = Math.pow(uy-vy, 2);
      double distance = Math.pow((xcoordinate+ycoordinate), 0.5); //square root
      return distance;
  }
  ...
  public void computeAllEuclideanDistances() {
      for (String m: vertexNames.keySet()){
          for (Edge e: vertexNames.get(m).adjacentEdges){
  ...
  public void doDijkstra(String s) {
      ...
      //initialize starting index and omit starting city from list
      int index = toCheck.indexOf(s);
      Vertex check = getVertex(toCheck.remove(index));
      
      
      for (int i=0; i < toCheck.size(); i++){
          Vertex x = getVertex(toCheck.get(i));
          x.distance = 5000000; //set to arbitrary number at start 
      }
      
      check.distance = 0;
      for (Edge e: check.adjacentEdges){
          Vertex target = e.target;
          int modifyIndex = toCheck.indexOf(target.name);
          Vertex modify = getVertex(toCheck.get(modifyIndex));
          modify.prev = check;
          modify.distance = check.distance + e.distance;
      }
      
      while (toCheck.isEmpty() == false){
          double initial = 5000000;
          int indexRemove = 0;
          for (int i=0; i< toCheck.size(); i++){
              Vertex v = getVertex(toCheck.get(i));
              if (v.distance < initial){
                  initial = v.distance;
                  indexRemove = i;
              }
          }
          check = getVertex(toCheck.remove(indexRemove));
      
      
          for (Edge e: check.adjacentEdges){
              Vertex target = e.target;
              if (toCheck.contains(target.name)){
                  int modifyIndex = toCheck.indexOf(target.name);
                  Vertex modify = getVertex(target.name);
                  double temp = check.distance + e.distance;
                  if (modify.distance > temp){
                      modify.distance = temp;
                      modify.prev = check;
                  }
              }
          }
  ...
  public List<Edge> getDijkstraPath(String s, String t){
      for (Vertex x: getVertices()){
          x.distance = 5000000; //arbitrary distance
          x.prev = null;
      }
      doDijkstra(s);
      
      List<Edge> myPath = new ArrayList<>();
